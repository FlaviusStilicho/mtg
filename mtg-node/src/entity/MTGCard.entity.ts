...
export class MTGCard {
  ...
  isStandardLegal(): boolean {
    return (
      this.versions.filter((version) => version.isStandardLegal).length > 0
    );
  }

  isCommanderLegal(): boolean {
    return (
      this.versions.filter((version) => version.isCommanderLegal).length > 0
    );
  }
  ...
  toDTO() {
    ...
    if (
      result.versions.filter((version) => version.isPrimaryVersion).length == 0
    ) {
  ...
  getPrimaryVersion(): MTGCardVersion {
    const primaryVersions: MTGCardVersion[] = this.versions.filter(
  ...
  getVersion(scryfallId: string): MTGCardVersion {
    const primaryVersions: MTGCardVersion[] = this.versions.filter(
      (version) => version.scryfallId == scryfallId,
    );
  ...
  static async Query(
    take: number,
    page: number,
    params: CardQueryParameters,
  ): Promise<[MTGCard[], number]> {
    var qb = this.repo
    ...
    if (params.cardName) {
      qb.andWhere(
        `(card.name like "%${params.cardName}%" or card.backSideName like "%${params.cardName}%")`,
      );
    }
    if (params.cardText) {
      var words: string[] = params.cardText
        .replaceAll('"', "'")
        .match(/"(?:\\"|[^"])*"|'(?:\\'|[^'])*'|[^\s"']+/g);
      ...
      if (words.length === 1) {
        if (words[0].startsWith("!")) {
          qb.andWhere(
            `(card.text not like "%${words[0].substring(
              1,
            )}%" and card.backSideText not like "%${words[0].substring(1)}%")`,
          );
        } else {
          qb.andWhere(
            `(card.text like "%${words[0]}%" or card.backSideText like "%${words[0]}%")`,
          );
        }
      } else {
        words
          ...
          .map((word) =>
            qb.andWhere(
              `(card.text like "%${word}%" or card.backSideText like "%${word}%")`,
            ),
          );
        words
          .filter((word) => word.startsWith("!"))
          .map((word) =>
            qb.andWhere(
              `(card.text not like "%${word.substring(
                1,
              )}%" and card.backSideText not like "%${word.substring(1)}%")`,
            ),
          );
      }
    }
    ...
    if (params.rarities && params.rarities.length < 4) {
      qb.andWhere("card.rarity IN(:...rarities)", {
    ...
    if (params.subType || params.types) {
      ...
      if (words.length === 1) {
        if (words[0].startsWith("!")) {
          qb.andWhere(
            `(card.type not like "%${words[0].substring(
              1,
            )}%" and card.backSideType not like "%${words[0].substring(1)}%")`,
          );
        } else {
          qb.andWhere(
            `(card.type like "%${words[0]}%" or card.backSideType like "%${words[0]}%")`,
          );
        }
      } else {
        if (params.typeSearchSetting == "Includes at least") {
          ...
          words
            ...
            .map((word) =>
              qb.andWhere(
                `(card.type like "%${word}%" or card.backSideType like "%${word}%")`,
              ),
            );
          words
            .filter((word) => word.startsWith("!"))
            .map((word) =>
              qb.andWhere(
                `(card.type not like "%${word.substring(
                  1,
                )}%" and card.backSideType not like "%${word.substring(1)}%")`,
              ),
            );
    ...
    if (params.colors) {
      if (params.colorSearchSetting == "Exact match") {
        params.colors.map((color) =>
          qb.andWhere(`card.colorIdentity like "%${color}%"`),
        );
        allColors
          .filter((color) => params.colors.indexOf(color.name) === -1)
          .map((color) =>
    ...
    if (params.minOwnedCopies && params.minOwnedCopies > 0) {
      qb.andWhere("card.ownedCopies >= :copies", {
        copies: params.minOwnedCopies,
      });
    }
...
export async function AddCategory(cardId: number, category: string) {
  ...
  if (categoryInstance) {
    ...
    logger.debug(
      `Added category ${category} to card ${card.name}. Created category ${category}.`,
    );
  }
}
...
export async function RemoveCategory(cardId: number, categoryName: string) {
  ...
  const extraCategories = card.tags.filter((category) => {
    return category.name !== categoryName;
  });

  if (extraCategories.length === card.tags.length) {
    logger.debug(
      `Category ${categoryName} did not exist on card ${card.name}.`,
    );
  } else {
    card.tags = extraCategories;
    await DB.manager.save(MTGCard, card);
...
export class MTGCardVersion {
  static repo: Repository<MTGCardVersion> =
    DB.manager.getRepository(MTGCardVersion);
  ...
  constructor(
    ...
    (this.set = set), (this.isPrimaryVersion = isPrimaryVersion);
  ...
  toDTO() {
    return <MTGCardVersionDTO>{
      ...
      backImageUri: this.backIllustrationId
        ? `http://localhost:8000/cards/${this.backIllustrationId}.jpg`
        : null,
    };
  }
}
