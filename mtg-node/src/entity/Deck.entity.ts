...
export class Deck {
  ...
  static async fromDTO(dto: DeckDTO): Promise<Deck> {
    ...
    const entries = await dto.cardEntries.map((entry) =>
      DeckCardEntry.fromDTO(entry),
    );
  ...
  verifyDeckLegal(rules: DeckRules): [boolean, string] {
    try {
      const numberOfCards = this.cardEntries
        .map((entry) => entry.copies)
        .reduce((a, b) => a + b);
      if (numberOfCards > rules.cardMaximum)
        throw new Error(
          `Deck contains too many cards. Maximum allowed in format ${rules.formatName}: ${rules.cardMaximum}. Currently contains: ${numberOfCards}`,
        );
      if (numberOfCards < rules.cardMinimum)
        throw new Error(
          `Deck contains too few cards. Minimum allowed in format ${rules.formatName}: ${rules.cardMinimum}. Currently contains: ${numberOfCards}`,
        );
      this.cardEntries.forEach((entry) => {
        if (
          entry.card.type !== "Basic Land" &&
          entry.copies < rules.maximumCardCopies
        ) {
          throw new Error(
            `Deck contains too many copies of card ${entry.card.name}. Maximum allowed in format ${rules.formatName}: ${rules.maximumCardCopies}. Currently contains: ${entry.copies}`,
          );
        }
      });
      return [true, null];
