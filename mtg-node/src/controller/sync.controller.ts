  ...
  return await MTGCardRepository.insert(newCards).then(async () => {
    ...
    return {
      ...
      updatedCards: Array.from(cardsMap.values()).filter(
        (entry) => entry[1] === CardState.Updated,
      ).length,
      newCardsInserted: Array.from(cardsMap.values()).filter(
        (entry) => entry[1] === CardState.New,
      ).length,
      unchangedCards: Array.from(cardsMap.values()).filter(
        (entry) => entry[1] === CardState.Unchanged,
      ).length,
    };
  });
};

async function fetchCardsFromScryfall(
  sets: MTGSet[],
...
function ParseAndAttachCardVersion(
  rawCard: object,
  sets: MTGSet[],
  cardsMap: Map<string, [MTGCard, CardState]>,
) {
