...
export const CreateBuylistMultipleDecks = async (
  req: Request<{}, {}, ExportDecksParams, {}>,
  res: Response,
) => {
  ...
  const cardEntriesLists: DeckCardEntry[] = decks
    .map((deck) => deck.cardEntries)
    .flat();
  const cardIds = new Set(cardEntriesLists.map((entry) => entry.card.id));
  const cardEntriesMap = new Map<number, object>();

  MTGCardRepository.findBy({ id: In(Array.from(cardIds)) }).then(
    (cards: MTGCard[]) => {
