...
export const GetWishlistEntries = async (
  req: Request,
  res: Response<WishlistEntryDTO[]>,
) => {
  ...
  Promise.all(dtos.map((entry) => WishlistEntry.fromDTO(entry))).then(
    async (newWishlistEntries: WishlistEntry[]) => {
      await WishlistEntryRepository.find().then((currentWishlistEntries) => {
        ...
        const newWishlistEntryCardIds: Set<number> = new Set(
          newWishlistEntries.map((entry) => entry.card.id),
        );
        const wishlistEntriesToDelete: WishlistEntry[] =
          currentWishlistEntries.filter(
            (entry) => !newWishlistEntryCardIds.has(entry.card.id),
          );

        logger.info(
          `deleting ${wishlistEntriesToDelete.length} wishlist entries`,
        );
        wishlistEntriesToDelete.forEach((entry) =>
          WishlistEntryRepository.delete(entry.id),
        );
      });
    },
  );
};
export const AddWishlistEntry = async (
  req: Request<{}, {}, WishlistEntryDTO, {}>,
  res: Response,
) => {
  const cardName = req.body.card.name;
  await WishlistEntryRepository.createQueryBuilder("wishlistEntry")
...
export const RemoveWishlistEntry = async (
  req: Request<{}, {}, WishlistEntryDTO, {}>,
  res: Response,
) => {
  const cardName = req.body.card.name;
