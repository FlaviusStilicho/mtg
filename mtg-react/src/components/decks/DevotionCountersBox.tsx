...
export class DevotionCountersBox extends Component<DevotionCountersBoxProps> {
  shouldComponentUpdate(nextProps: DevotionCountersBoxProps) {
    const currentCards = new Set(
      this.props.entries.map((entry) => entry.card.name),
    );
    const nextCards = new Set(
      nextProps.entries.map((entry) => entry.card.name),
    );
    if (!areSetsEqual(currentCards, nextCards)) {
      return true;
    }
    const currentCardCount: number =
      this.props.entries.length > 0
        ? this.props.entries
            .map((entry) => entry.copies)
            .reduce((a, b) => a + b)
        : 0;
    const nextCardCount: number =
      nextProps.entries.length > 0
        ? nextProps.entries.map((entry) => entry.copies).reduce((a, b) => a + b)
        : 0;
  ...
  splitBraces(input: string): string[] {
    ...
    return matches
      ? matches.map((str) => str.substring(1, str.length - 1))
      : [];
  }
  ...
  render() {
    ...
    const totalNumberOfCardsInDeck =
      this.props.entries.length > 0
        ? this.props.entries
            .map((entry) => entry.copies)
            .reduce((a, b) => a + b)
        : 0;
    colorDevotionMap.average =
      colorDevotionMap.total / totalNumberOfCardsInDeck;

    return this.props.entries.length > 0 ? (
      <List>
        <Box
          style={{ textAlign: "left", marginLeft: 25 }}
          sx={colorDevotionBoxStyle}
        >
          Color devotion:
        </Box>
        {colorDevotionMap.white > 0 ? (
          <Box
            style={{ textAlign: "left", marginLeft: 25 }}
            sx={colorDevotionBoxStyle}
          >
            <ColorIcon color="W" key="wD" /> {colorDevotionMap.white}
          </Box>
        ) : (
          <></>
        )}
        {colorDevotionMap.blue > 0 ? (
          <Box
            style={{ textAlign: "left", marginLeft: 25 }}
            sx={colorDevotionBoxStyle}
          >
            <ColorIcon color="U" key="UD" /> {colorDevotionMap.blue}
          </Box>
        ) : (
          <></>
        )}
        {colorDevotionMap.black > 0 ? (
          <Box
            style={{ textAlign: "left", marginLeft: 25 }}
            sx={colorDevotionBoxStyle}
          >
            <ColorIcon color="B" key="BD" /> {colorDevotionMap.black}
          </Box>
        ) : (
          <></>
        )}
        {colorDevotionMap.red > 0 ? (
          <Box
            style={{ textAlign: "left", marginLeft: 25 }}
            sx={colorDevotionBoxStyle}
          >
            <ColorIcon color="R" key="RD" /> {colorDevotionMap.red}
          </Box>
        ) : (
          <></>
        )}
        {colorDevotionMap.green > 0 ? (
          <Box
            style={{ textAlign: "left", marginLeft: 25 }}
            sx={colorDevotionBoxStyle}
          >
            <ColorIcon color="G" key="GD" /> {colorDevotionMap.green}
          </Box>
        ) : (
          <></>
        )}
        {colorDevotionMap.colorless > 0 ? (
          <Box
            style={{ textAlign: "left", marginLeft: 25 }}
            sx={colorDevotionBoxStyle}
          >
            {" "}
            <ColorIcon color="C" key="CD" /> {colorDevotionMap.colorless}
          </Box>
        ) : (
          <></>
        )}
        <Box
          ...
          sx={colorDevotionBoxStyle}
        >
          Average mana cost: {colorDevotionMap.average.toFixed(1)}
        </Box>
      </List>
    ) : (
      <></>
