...
export class MTGCardComponent extends Component<
  CardComponentProps,
  CardComponentState
> {
  constructor(props: CardComponentProps) {
    ...
    const primaryVersion: MTGCardVersionDTO = props.card.versions.filter(
      (version) => version.isPrimaryVersion,
    )[0];
    const variantVersions: MTGCardVersionDTO[] = props.card.versions.filter(
      (version) => version.isPrimaryVersion === false,
    );
  ...
  shouldComponentUpdate(
    ...
    return shouldCardComponentUpdate(
      this.props,
      nextProps,
      this.state,
      nextState,
  ...
  flipCard = (): void => {
    const primaryImage = this.props.card.versions
      .filter((version) => version.isPrimaryVersion)
      .map((version) =>
        this.state.frontSideUp ? version.frontImageUri : version.backImageUri,
      )
      .filter(this.notEmpty)[0];
    this.setState({ primaryImage });
    const variantImages: string[] = this.props.card.versions
      .filter((version) => version.isPrimaryVersion === false)
      .map((version) =>
        this.state.frontSideUp ? version.frontImageUri : version.backImageUri,
      )
  ...
  fetchSellPrice = (): void => {
    if (this.state.sellPrice === "") {
      ...
      axios
        .post(
          "https://www.magicers.nl/inc/ajax_public/buylist.php",
        ...
        .then((response) => {
          ...
          try {
            root.childNodes[3].childNodes.forEach((node: any) => {
              if (
                "rawAttributes" in node &&
                "id" in node.rawAttributes &&
                node.rawAttributes["id"].startsWith("productrow")
              ) {
                node.childNodes.forEach((childNode: any) => {
                  if (
                    "rawAttributes" in childNode &&
                    "id" in childNode.rawAttributes &&
                    childNode.rawAttributes["id"].startsWith("price")
                  ) {
                    const price = Number(
                      childNode.rawAttributes["data-text"].replace(",", "."),
                    );
                    if (price > 0) {
                      prices.push(price);
                    }
  ...
  render() {
    ...
    const deckCounterBoxProps: MTGCardDeckCounterBoxProps = {
      ...
      primaryVersion: this.props.card.versions.filter(
        (version) => version.isPrimaryVersion,
      )[0],
      ...
      >
        ...
        {this.props.enabledTab === EnabledTab.COLLECTION && (
          <MTGCardCollectionCounterBox
            card={this.props.card}
            primaryVersion={
              this.props.card.versions.filter(
                (version) => version.isPrimaryVersion,
              )[0]
            }
