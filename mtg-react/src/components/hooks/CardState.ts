...
export const useCardState = (card: MTGCardDTO) => {
  const primaryVersion: MTGCardVersionDTO = card.versions.filter(
    (version) => version.isPrimaryVersion,
  )[0];
  const variantVersions: MTGCardVersionDTO[] = card.versions.filter(
    (version) => version.isPrimaryVersion === false,
  );

  const [frontSideUp, setFrontSideUp] = useState<boolean>(true);
  const [primaryImage, setPrimaryImage] = useState<string>(
    primaryVersion.frontImageUri,
  );
  ...
  function flipCard(): void {
    const primaryImage = card.versions
      .filter((version) => version.isPrimaryVersion)
      .map((version) =>
        frontSideUp ? version.frontImageUri : version.backImageUri,
      )
      .filter(notEmpty)[0];
    setPrimaryImage(primaryImage);
    const variantImages: string[] = card.versions
  ...
  const postUpdatedOwnedCopies = useConstant(() =>
    debounce((body: UpdateCardOwnedCopiesQueryParams) => {
