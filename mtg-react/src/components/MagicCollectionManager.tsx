...
export class MagicCollectionManager extends Component<
...
> {
  ...
  fetchCards(
    queryParameters: CardQueryParameters,
  ...
  render() {
    ...
    const handleChangeSelectedQueryParameters = (
      event: SelectChangeEvent<CardQueryParameters>,
    ) => {
      ...
      if (propName === "cardName" && typeof newValue === "string") {
        queryParameters = {
          ...queryParameters,
          cardName: newValue.trim().toLowerCase(),
        };
      } else if (propName === "cardText" && typeof newValue === "string") {
        queryParameters = {
          ...queryParameters,
          cardText: newValue.trim().toLowerCase(),
        };
      } else if (
        propName === "sets" &&
        Array.isArray(newValue) &&
        newValue.every((item) => typeof item === "number")
      ) {
        var newSets: number[] = [];
        if (newValue[newValue.length - 1] === 99999) {
          newSets =
            queryParameters.sets.length > 0
              ? []
              : this.state.sets.map((set) => set.id);
        } else if (newValue[newValue.length - 1] === 99998) {
          newSets = this.state.sets
            .filter((set) => new Date(set.releaseDate) > new Date("2020-01-01"))
            .map((set) => set.id);
        } else {
          newSets = newValue;
        }
        ...
        queryParameters = {
          ...queryParameters,
          subType: newValue.trim().toLowerCase(),
        };
      } else if (
        propName === "colors" &&
        Array.isArray(newValue) &&
        ...
        queryParameters = {
          ...queryParameters,
          manaCost: newValue.trim().toLowerCase(),
        };
        ...
        propName === "minOwnedCopies" &&
        typeof newValue === "string"
      ) {
        queryParameters = {
          ...queryParameters,
          minOwnedCopies: parseInt(newValue),
        };
      } else {
        console.error(
          `Type error while updating query parameters: field=${propName}, type=${typeof newValue}, value=${newValue}`,
        );
      }
    ...
    const handleChangeSelectedDeck = (event: SelectChangeEvent<number>) => {
      ...
      if (typeof newDeckId === "string") {
        ...
        const newDeck = this.state.decks.filter(
          (deck) => deck.id === newDeckId,
        )[0];
        const colorIdentity: string[] = Array.from(
          getDeckColorIdentity(newDeck),
        );
          ...
          selectedQueryParameters: {
            ...
            colorSearchSetting:
              colorIdentity.length > 0
                ? "Includes at most"
                : this.state.selectedQueryParameters.colorSearchSetting,
          },
        });

        console.log("Collecting card prices");
        Promise.all(
          getDeck(newDeckId).cardEntries.map((entry) => {
            if (
    ...
    const getExistingEntry = (newCard: MTGCardDTO) => {
      const existingCardEntry: DeckCardEntryDTO[] =
        this.state.selectedDeckEntries.filter(
          (entry) => entry.card.id === newCard.id,
        );
    ...
    const getCurrentNumberOfCopiesForCard = (
      card: MTGCardDTO,
    ...
    const updateDeckEntries = (entry: DeckCardEntryDTO) => {
      ...
      newEntriesList = currentEntriesList
        .filter((currentEntry) => currentEntry.card.name !== entry.card.name)
    ...
    const updateCardCopiesInDeck = (
      ...
      if (!existingCardEntry) {
        ...
        newEntry = {
          ...
          card: card,
          copies: isSideboard
            ? existingCardEntry.copies
            : existingCardEntry.copies + increment,
          sideboardCopies: isSideboard
            ? existingCardEntry.sideboardCopies + increment
            : existingCardEntry.sideboardCopies,
          isCommander: false,
        };
    ...
    function checkEntryIllegal(entry: DeckCardEntryDTO, deck: DeckDTO) {
      ...
      if (isBasicLand(entry.card)) {
        ...
        entry.copies > maxCardCopies ||
        entry.sideboardCopies > maxCardCopies
      ) {
    ...
    const postUpdatedOwnedCopies = debounce(
      (body: UpdateCardOwnedCopiesQueryParams) => {
    ...
    const updateCardCopiesInWishlist = async (
      card: MTGCardDTO,
      add: boolean,
    ) => {
      var updatedEntries: WishlistEntryDTO[] =
        this.state.wishlistEntries && this.state.wishlistEntries.length > 0
      ...
      if (matchingEntry.length === 1) {
        ...
        entry.desiredCopies = add
          ? entry.desiredCopies + 1
          : entry.desiredCopies - 1;
        if (entry.desiredCopies <= entry.card.ownedCopies) {
          ...
          updatedEntries = this.state.wishlistEntries.filter(
          ...
          openInfoSnackbar(
            `Set desired copies for ${card.name} on wishlist to ${entry.desiredCopies}`,
          );
        }
      } else {
        if (!add) {
          throw Error(
            "Cannot decrease number of copies on card that isnt on wishlist",
          );
        }
        if (!card.priceInfo) {
      ...
      axios
        .post(`http://localhost:8000/wishlist/`, updatedEntries)
        .then((response) => {
    ...
    const navBarProps: NavBarProps = {
      ...
      collectionUploadWindowOpened: this.state.collectionUploadWindowOpened,
      handleCollectionUploadWindowOpenClose:
        this.handleCollectionUploadWindowOpenClose,
          ...
          <Box width="100%">
            ...
            >
              <Alert severity="info" sx={{ width: "100%" }}>
                {this.state.snackbarInfoMessage}
              </Alert>
            </Snackbar>
          </Box>
        </Box>
