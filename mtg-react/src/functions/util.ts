...
export const filterPlaneswalkers = (
  entries: DeckCardEntryDTO[],
): DeckCardEntryDTO[] => {
  return entries
    .filter((entry) => entry.copies > 0)
...
export const filterNoncreatureArtifacts = (
  entries: DeckCardEntryDTO[],
): DeckCardEntryDTO[] => {
  return entries
    .filter((entry) => entry.copies > 0)
    .filter(
      (entry) => isArtifact(entry) && !isCreature(entry) && !isLand(entry),
    )
    .sort(sortDeckEntriesFn);
};

export const filterNoncreatureEnchantments = (
  entries: DeckCardEntryDTO[],
): DeckCardEntryDTO[] => {
  return entries
    .filter((entry) => entry.copies > 0)
    .filter(
      (entry) => isEnchantment(entry) && !isCreature(entry) && !isLand(entry),
    )
    .sort(sortDeckEntriesFn);
};

export const filterSorceries = (
  entries: DeckCardEntryDTO[],
): DeckCardEntryDTO[] => {
  return entries
    .filter((entry) => entry.copies > 0)
...
const sortDeckEntriesFn = (
  entry1: DeckCardEntryDTO,
  entry2: DeckCardEntryDTO,
) => {
...
export const getNumberOfPlaneswalkers = (
  entries: DeckCardEntryDTO[],
): number => {
  return getTotalCardCopies(filterPlaneswalkers(entries));
};

export const getNumberOfNoncreatureArtifacts = (
  entries: DeckCardEntryDTO[],
): number => {
  return getTotalCardCopies(filterNoncreatureArtifacts(entries));
};

export const getNumberOfNoncreatureEnchantment = (
  entries: DeckCardEntryDTO[],
): number => {
...
export const getNumberOfSideboardCards = (
  entries: DeckCardEntryDTO[],
): number => {
  return entries.length > 0
    ? entries
...
export const isCommanderEligible = (entry: DeckCardEntryDTO): boolean => {
  if (entry.isCommander === true || entry.copies === 0) {
    ...
    entry.card.type.includes("Legendary") &&
    entry.card.type.includes("Creature")
  ) {
    return true;
...
export const numberOfMissingCards = (
  entry: DeckCardEntryDTO,
  isSideboardEntry: boolean = false,
): number => {
  if (!isSideboardEntry) {
...
export const numberOfCardsAvailable = (
  entries: DeckCardEntryDTO[],
): [number, number] => {
  if (entries.length === 0) {
    return [0, 0];
...
export function getCommander(deck: DeckDTO | null) {
  ...
  const currentCommanderEntries: DeckCardEntryDTO[] = deck.cardEntries.filter(
    (entry) => entry.isCommander,
  );
...
export function getDeckColorIdentity(deck: DeckDTO | null): Set<string> {
  ...
  if (commander) {
    ...
    deck.cardEntries.forEach((entry) =>
      entry.card.colorIdentity.forEach((color) => deckColorIdentity.add(color)),
    );
  }
...
export function wishlistSortFnAlphabetical(
  a: WishlistEntryDTO,
  b: WishlistEntryDTO,
): number {
...
export function wishlistSortFnPrice(
  a: WishlistEntryDTO,
  b: WishlistEntryDTO,
): number {
...
export function populateDeckWishlistEntryMap(
  wishlist: WishlistEntryDTO[],
): Map<string, WishlistEntryDTO[]> {
...
export function countCardsByRarity(
  entries: DeckCardEntryDTO[],
  rarity: string,
): number {
  return entries
    .filter((entry) => entry.card.rarity === rarity)
    .map((entry) => entry.copies)
    .reduce((a, b) => a + b, 0);
}

export function hasDeckEntryChanges(
  card: MTGCardDTO,
  prevEntries: DeckCardEntryDTO[],
  nextEntries: DeckCardEntryDTO[],
): boolean {
  var hasChanges = false;

  const matchingPreviousEntries = prevEntries.filter(
    (entry) => entry.card.id === card.id,
  );
  const matchingNextEntries = nextEntries.filter(
    (entry) => entry.card.id === card.id,
  );

  if (
    matchingPreviousEntries.length === 0 &&
    matchingNextEntries.length === 0
  ) {
    hasChanges = false;
  } else if (
    matchingPreviousEntries.length === 0 &&
...
export function shouldCardComponentUpdate(
  currProps: CardComponentProps,
  nextProps: CardComponentProps,
  currState: CardComponentState,
  nextState: CardComponentState,
): boolean {
  ...
  if (
    hasDeckEntryChanges(
      currProps.card,
      currProps.selectedDeckEntries,
      nextProps.selectedDeckEntries,
    )
  ) {
    hasChanges = true;
